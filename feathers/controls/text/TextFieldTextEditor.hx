...
class TextFieldTextEditor extends FeathersControl implements ITextEditor
{
	...
	private function refreshSnapshotParameters():Void
	{
		...
		var clipWidth:Float = this.actualWidth * Starling.current.contentScaleFactor * matrixToScaleX(HELPER_MATRIX);
		if(clipWidth < 0)
		{
			clipWidth = 0;
		}
		var clipHeight:Float = this.actualHeight * Starling.current.contentScaleFactor * matrixToScaleY(HELPER_MATRIX);
		if(clipHeight < 0)
		{
			clipHeight = 0;
		}
